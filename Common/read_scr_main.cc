...
enum argIndex{kBin, kIp, kFilePath, kNumCh};
using namespace HUL;
...
int main(int argc, char* argv[])
{
  if(1 == argc){
    std::cout << "Usage\n";
    std::cout << "read_scr [IP address] [Output file path] [# of scaler channel]" << std::endl;
    std::cout << " - Number of channel: # of system scaler + # of hit scaler" << std::endl;
    return 0;
  }// usage
  ...
  const uint32_t kNumReadCh   = atoi(argv[kNumCh]);
  ...
  std::cout << "#D: Read length is " << kReadLength << std::endl;

  RBCP::UDPRBCP udp_rbcp(kBoardIp, RBCP::gUdpPort, RBCP::DebugMode::kNoDisp);
  HUL::FPGAModule fpga_module(udp_rbcp);

  fpga_module.WriteModule(SCR::kAddrScrReset,0);
